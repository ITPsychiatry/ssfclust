...
# > grid_ips <- seq_len(folds) * step
# > grid_ips
# [1] 0.1105573 0.2211146 0.3316718 0.4422291 0.5527864
# > grid_alpha <- vips(grid_ips)
# > grid_alpha
# [1] 0.1242995 0.2838859 0.4962709 0.7928509 1.2360680
# > round(grid_alpha, 3)
# [1] 0.124 0.284 0.496 0.793 1.236
# 
# > (alpha_beta <- round(alpha_beta, 3))
# [1] 1.236
# 
# > (round(ips(alpha_beta), 3))
# [1] 0.553
# 
# > (round((step <- 0.553 / 5), 3))
# [1] 0.111
# 
# > (grid_ips <- seq_len(folds) * .111)
# [1] 0.111 0.222 0.333 0.444 0.555
# 
# > (round(grid_alpha <- vips(grid_ips), 3))
# [1] 0.125 0.285 0.499 0.799 1.247
```
...
```{r}
Xs <- replicate(no_X_simulations, simulate_data(100)) |> asplit(3)

F_s <- vector(mode = "list", length = no_X_simulations)

for (i in seq_along(F_s)) {
  F_s[[i]] <- replicate(no_F_simulations, simulate_supervision(100)) |> asplit(3)
}

alphas_B <- list(0.3, 0.5, 0.7, 0.9, 1.)
alphas_ref <- as.list(seq(0, 3.5, length.out=ref_length))

ips_B <- ips(unlist(alphas_B))
ips_ref <- ips(unlist(alphas_ref))
ips_ref <- as.list(seq(0, 0.8, length.out = ref_length))   # TODO
# currently it's duck-typing, make it the only way after changes

alphas_ips05 <- establish_cv_grid(beta = 0.05)
alphas_ips2 <- establish_cv_grid(beta = 0.2)
```

## write exemplary dataset

```{r}
limits <- list(c(1, 100), c(101, 200), c(201, 300))

for (i in seq_along(limits)) {
  example_X <- Xs[[1]][limits[[i]][1] : limits[[i]][2], ]
  
  write.table(example_X,
            file = paste0("sample_data_c_", i, ".csv"), sep = ",", 
...
df_total <- simulate_for_grid(alphas_ref, Xs, F_s)
df_total_ref <- retrieve_evidence(df_total$outcomes_concat)
...
## `alphas_ips05`, `alphas_ips2` and `alphas_B`

```{r}
start_time <- Sys.time()

df_total <- simulate_for_grid(alphas_ips05, Xs, F_s)
df_total_ips05 <- retrieve_evidence(df_total$outcomes_concat)
df_total_ips05_v <- df_total$prototypes_concat
...
df_total <- simulate_for_grid(alphas_ips2, Xs, F_s)
df_total_ips2 <- retrieve_evidence(df_total$outcomes_concat)
df_total_ips2_v <- df_total$prototypes_concat
...
df_total <- simulate_for_grid(alphas_B, Xs, F_s)
df_total_B <- retrieve_evidence(df_total$outcomes_concat)
...
```{r}
df_sum_ips05 <- create_summary(df_total_ips05, ev)
df_sum_ips2 <- create_summary(df_total_ips2, ev)
df_sum_B <- create_summary(df_total_B, ev)
df_sum_ref <- create_summary(df_total_ref, ev)
```

Printing for latex text.

```{r}
with(df_sum_ips2,
     cat(paste0("($", round(alpha, 3), "$, $", round(me, 3), "$)", sep = ",")))
```
...
```{r}
ips_results05 <- df_sum_ips05
ips_results2 <- df_sum_ips2
...
```{r}
data_for_plot <- do.call(rbind, list(ips_results05, ips_results2, ref_results, B_results))
```

```{r}
save_dfs <- list(data_for_plot, ips_results05, ips_results2, B_results)
save_dfs_names <- paste0(c("all", "ips05", "ips2", "B"), ".csv")

for (idx in seq_along(save_dfs)) {
  write.table(arrange(save_dfs[[idx]], alpha) %>% mutate(ips = ips(alpha)),
              file = save_dfs_names[[idx]], sep = ",", 
              row.names = FALSE, quote = FALSE)
}
```

```{r}
df_for_print <- cbind(select(ips_results05, alpha, me),
                      select(ips_results2, alpha, me),
                      select(B_results, alpha, me))

napis <- ""

for (i in 1:5) {
  napis <- paste0(napis, "\n", i, " & ", paste0(round(df_for_print[i, ], 3), collapse = " & "), " \\\\ ")
}
...
# prototypes closeness


```{r}
calculate_se_stats <- function(df) {
  true_points <- data.frame(id = c(1, 2 ,3), V1 = c(5, 7, 9), V2 = c(5, 7, 9))
  
  closeness_basis <- merge(df, true_points, by = "id")
  closeness_basis$measure <- with(closeness_basis, sqrt((X1 - V1)^2 + (X2 - V2)^2))
  
  closeness_summary <- create_summary(closeness_basis, measure)
  closeness_summary$alpha <- unlist(closeness_summary$alpha)  
  # TODO why I need to unlist?
  
  return(closeness_summary)
}
```
...
```{r}
v_dfs <- list(
  do.call(rbind,list(df_total_ref_v, df_total_ips05_v, df_total_ips2_v, df_total_B_v)),
  df_total_ips05_v,
  df_total_ips2_v,
  df_total_B_v
)

v_dfs_names <- paste0(c("cs_all", "cs_ips05", "cs_ips2", "cs_B"), ".csv")

for (idx in seq_along(v_dfs)) {
  write.table(arrange(calculate_se_stats(v_dfs[[idx]]), alpha) %>% mutate(ips = ips(alpha)),
              file = v_dfs_names[[idx]], 
              sep = ",", 
              row.names = FALSE, quote = FALSE)
}
```
...
```{r}
# > df_total_ips2_v$alpha |> unique()
# [1] 0.1242995 0.2838859 0.4962709 0.7928509 1.2360680

df_total_ips2_v$group <- as.factor(round(df_total_ips2_v$alpha, 3))

chosen_results <- rbind(
  df_total_ref_v %>%
    filter(alpha == 0) %>%
    group_by(id) %>%
...
chosen_results_formatted <-
chosen_results %>%
  mutate(X1 = paste0(r"{ \makecell{ }",
                     as.character(round(x1_mean, 2)), 
                     r"{ \\ }", 
                     paste0("(", as.character(round(x1_sd, 2)), ")"),
                     r"{ } }"),
         X2 = paste0(r"{ \makecell{ }",
                     as.character(round(x2_mean, 2)), 
                     r"{ \\ }", 
                     paste0("(", as.character(round(x2_sd, 2)), ")"),
                     r"{ } }"),
         ips_napis = paste0(r"{ \makecell[{{p{0cm}}}]{\\[15pt]} }", ips)) %>%
  select(id, ips_napis, X1, X2) %>%
  tidyr::pivot_wider(names_from = id, values_from = c(X1, X2)) %>%
  select(ips_napis, X1_1, X2_1, X1_2, X2_2, X1_3, X2_3)

print(chosen_results_formatted)
```
...
for (i in 1:3) {
  napis <- paste0(napis, "\n", paste0(chosen_results_formatted[i, ], collapse = " & "), " \\\\ \\hline")
}

cat(napis)
```

We resigned from that plot, that's why it is commented out below.

```{r}
# save_dfs <- list(closeness_basis, df_total_ips05_v, df_total_ips2_v, df_total_B_v)
# save_dfs_names <- paste0(c("cs_all", "cs_ips05", "cs_ips2", "cs_B"), ".csv")
# 
# for (idx in seq_along(save_dfs)) {
#   write.table(arrange(
#     create_summary(save_dfs[[idx]], measure),
#     alpha) ,#%>% mutate(ips = ips(alpha)),
#               file = save_dfs_names[[idx]], sep = ",", 
#               row.names = FALSE, quote = FALSE)
# }
...
#   ggsave(filename = save_names[[idx]],
#          plot=save_plots[[idx]],
#          #width=3.5,
#          width=3.5,
#          #height=2,
